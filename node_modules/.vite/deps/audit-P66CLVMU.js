...
var labelableElements = ["button", "input", "meter", "output", "progress", "select", "textarea"];
  ...
  },
  {
    ...
    match(element) {
      ...
      for (const elementRole of elementRoles) {
        const roleData = import_aria_query.roles.get(normalizeAriaRole(elementRole));
  ...
  },
  {
    ...
    match(element) {
      ...
      for (const elementRole of elementRoles) {
        ...
        const unsupportedAttributes = import_aria_query.aria.keys().filter((attribute) => !(attribute in props));
  ...
  for (const [schema, ax_object] of import_axobject_query.elementAXObjects.entries()) {
    if (schema.name === tag_name && (!schema.attributes || schema.attributes.every((attr) => attributes[attr.name] === attr.value))) {
      for (const name of ax_object) {
        const axRoles = import_axobject_query.AXObjectRoles.get(name);
        if (axRoles) {
...
var dynamicAuditRuleKeys = ["title", "message", "description"];
...
function createRoundedBadge(icon2) {
  ...
  badge.shadowRoot.innerHTML += `
	...
	`;
  badge.innerHTML = `<astro-dev-toolbar-icon icon="${icon2}"></astro-dev-toolbar-icon>0`;
  return {
    badge,
    updateCount: (count) => {
      if (count === 0) {
      ...
      badge.innerHTML = `<astro-dev-toolbar-icon icon="${icon2}"></astro-dev-toolbar-icon>${count}`;
    }
  };
}
...
//# sourceMappingURL=audit-P66CLVMU.js.map
